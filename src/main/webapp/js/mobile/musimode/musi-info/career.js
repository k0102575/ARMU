...
if (spno == 0) {
  ...
  for(var i = 0; i < result.data.getSpec.list.length; i++) {
    if(result.data.getSpec.list[i].fileMap.key == "N") {
      ...
      $(".movie-container").append("<iframe width='791' height='876' src='https://www.youtube.com/embed/" + sourceMovie + "?rel=0&amp;showinfo=0' frameborder='0' allowfullscreen></iframe>")
    }
  }
  ...
  $("#movie-text-btn").on('click', function() {
    ...
    if ($("#movie-text").val().indexOf(findStr) != -1) {
      ...
      $(".movie-container").append("<iframe width='791' height='876' src='https://www.youtube.com/embed/" + $("#movie-text").val().substring(17,28) + "?rel=0&amp;showinfo=0' frameborder='0' allowfullscreen></iframe>")
      $("#movie-text").val('')
    }
