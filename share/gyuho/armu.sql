...
CREATE TABLE musi (
	...
	pay   INTEGER      NOT NULL COMMENT '희망 페이', -- 희망 페이
	age   INTEGER      NOT NULL COMMENT '연령대', -- 연령대
...
ALTER TABLE musi
	MODIFY COLUMN muno INTEGER NOT NULL AUTO_INCREMENT COMMENT '뮤지션회원번호';
...
ALTER TABLE evn
	MODIFY COLUMN gno INTEGER NOT NULL AUTO_INCREMENT COMMENT '일반회원번호';
...
CREATE TABLE spec (
	...
	desc TEXT    NOT NULL COMMENT '설명' -- 설명
)
COMMENT '경력/이력';

-- 경력/이력
ALTER TABLE spec
	ADD CONSTRAINT PK_spec -- 경력/이력 기본키
		PRIMARY KEY (
			spno -- 경력이력번호
		);
...
CREATE TABLE team (
	muno INTEGER     NOT NULL COMMENT '뮤지션회원번호', -- 뮤지션회원번호
	num  INTEGER     NOT NULL COMMENT '인원수', -- 인원수
...
-- 매칭
CREATE TABLE match (
	matno INTEGER NOT NULL COMMENT '매칭 번호', -- 매칭 번호
	muno  INTEGER NOT NULL COMMENT '뮤지션회원번호', -- 뮤지션회원번호
	eno   INTEGER NOT NULL COMMENT '이벤트 번호', -- 이벤트 번호
	matdt DATE    NOT NULL COMMENT '매칭확정날짜', -- 매칭확정날짜
...
CREATE TABLE chat (
	...
	muno   INTEGER NOT NULL COMMENT '뮤지션회원번호', -- 뮤지션회원번호
	mno    INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
	read   CHAR(1) NOT NULL COMMENT '읽기여부', -- 읽기여부
...
-- 경력/이력
ALTER TABLE spec
	ADD CONSTRAINT FK_musi_TO_spec -- 뮤지션회원 -> 경력/이력
		FOREIGN KEY (
			muno -- 뮤지션회원번호
		)
...
-- 경력 파일
ALTER TABLE spec_path
	ADD CONSTRAINT FK_spec_TO_spec_path -- 경력/이력 -> 경력 파일
		FOREIGN KEY (
			spno -- 경력이력번호
		)
		REFERENCES spec ( -- 경력/이력
			spno -- 경력이력번호
		);
...
-- 매칭
ALTER TABLE match
	ADD CONSTRAINT FK_musi_TO_match -- 뮤지션회원 -> 매칭
		FOREIGN KEY (
			muno -- 뮤지션회원번호
		)
		REFERENCES musi ( -- 뮤지션회원
...
-- 매칭
ALTER TABLE match
	ADD CONSTRAINT FK_evn_TO_match -- 이벤트 -> 매칭
		FOREIGN KEY (
			eno -- 이벤트 번호
		)
		REFERENCES evn ( -- 이벤트
